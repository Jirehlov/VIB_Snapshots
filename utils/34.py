...
def main(HOW_OLD, json_file):
    df = pd.read_csv("sorted1.csv", encoding='utf-8')
    ...
    def to_offset(fc):
        return next((i for i, t in enumerate([10, 50, 100, 200, 500, 1000]) if fc < t), 6)
    times = 0
    for index, row in df.iterrows():
        ...
        if subject_id in subjects_info:
            ...
            if rate != 0:
                ...
                val4 = row.iloc[159 + to_offset(frequency_counters[subject_type][2]) - rate * 7]
                val5 = row.iloc[228 + HOW_OLD  - rate * 7]
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='叛逆指数计算。请先运行32.py获取个人收藏json以及VIB数据csv')
