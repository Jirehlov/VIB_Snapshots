...
sorted_counts = pd.read_csv('sorted1.csv')['subject'].value_counts().groupby(lambda x: x // 10000).sum()
max_subject_id = max(pd.read_csv('C:\\Users\\contact\\Documents\\GitHub\\VIB_Snapshots\\server_backup\\skip_counts.csv')['subject_id'].max(), pd.read_csv('sorted1.csv')['subject'].max())
...
total_other = max_subject_id - total_skip - total_sorted
